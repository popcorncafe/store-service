...
public class IngredientMapper implements Mapper<Ingredient, IngredientDto> {
    ...
    public Ingredient toModel(IngredientDto dto) {
        return new Ingredient(dto.id(), dto.name(), dto.unitPrice(), Ingredient.Measure.valueOf(dto.measure()));
    }

    @Override
    public IngredientDto toDto(Ingredient model) {
        return new IngredientDto(model.ingredientId(), model.name(), model.unitPrice(), model.measure().name());
    }
}
